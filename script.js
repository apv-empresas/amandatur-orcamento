(function () {
  ...
  function cacheElements() {
    [
      ...
      "heroTotal", "heroBgImage", "busImage"
    ].forEach((id) => {
  ...
  function applyHeroImage() {
    ...
    const busImageUrl = CONFIG.brand?.busImageUrl;

    if (imageUrl && elements.heroBgImage) {
      elements.heroBgImage.style.backgroundImage = `url(${JSON.stringify(imageUrl)})`;
      elements.heroBgImage.classList.add("active");
    }

    if (busImageUrl && elements.busImage) {
      elements.busImage.src = busImageUrl;
      elements.busImage.alt = CONFIG.brand?.busImageAlt || "Ônibus AmandaTur";
    }
  }
